...
| GET | `/calls/{id}/download` | Скачать запись (по возможности — стерео `_mix.wav`) |
...
### Какой файл записи отдаётся

Имя файла берётся из CDR-таблицы (колонка `recordingfile` / `filename` / `userfield` — определяется автоматически), а каталог — из `astspooldir` в `/etc/asterisk/asterisk.conf` (`<astspooldir>/monitor`).

`MixMonitor` в режиме **STEREO** сохраняет на один звонок несколько файлов, например:
...
В CDR Asterisk пишет базовое имя (`<base>.wav`). Поэтому при скачивании API сначала проверяет наличие стерео-версии `<base>_mix.wav` и отдаёт **её**, если файл существует. Если стерео-файла нет — отдаётся исходный файл, записанный в CDR.
